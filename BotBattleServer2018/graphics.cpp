...
void Graphics::handleEvent(int x, int y, EvtType evtType, ModKey mods, int arg, int pluginId, const std::string& data)
{
    ...
    postEvent(x/drawScale, (_height-y)/drawScale, evtType, mods, arg, pluginId, data);
}
...
{
    _mousePos.x = x / drawScale;
    _mousePos.y = (_height-y) / drawScale;
}
...
void Graphics::draw(QWidget *pd, QPainter *painter, int width, int height, int elapsed)
{
    ...
    drawScale = std::min((double)_width/initWidth, (double)_height/initHeight);
    ...
    if (!closed)
    {
        ...
        if (_width == initWidth && _height == initHeight) {
            drawScale = 1.0;
            painter->scale(1,1);
        }
        else {
...
{
    //cout  << "graphicsMain " << QThread::currentThreadId() << endl << endl;
    ...
    : QWidget(parent), _graphics(graphics), _parent(parent)
{
    ...
    startWidth = graphics->width();
    startHeight = graphics->height();

    //setMinimumHeight(graphics->height()/2);
    //setMinimumWidth(graphics->width()/2);
//    setBaseSize(graphics->width(), graphics->height());
...
{
    return QSize{startWidth, startHeight};
}
...
Window::Window(mssm::Graphics *g, std::string title)
{
    ...
    layout->addWidget(graphicsWidget, 1, 0);
...
void Window::appendOutputText(const std::string& txt)
{
    ...
    {
        streamOutBox->show();
        layout->addWidget(streamOutBox, 0, 0);
    }
...
{
    //cout  << "Music State Changed: " << static_cast<int>(state) << " Thread: " << QThread::currentThreadId() << endl;
